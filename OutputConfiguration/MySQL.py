def load_tableparametras(filename):
    ...
    create_allconf_table = create_allconf_table.format(filename=filename)
    create_diskconf_table = create_diskconf_table.format(filename=filename)
    
    return create_allconf_table, create_diskconf_table

def gen_execute_disk(List, tablename, filename):
    ...
    for i in range(0,disknumbs):
        execute_charters.append("INSERT INTO `" + filename + "`.`" + tablename + "`(`")
        values_charters_set = ""
        for i2 in defpar1:
    ...
    for i in range(0,disknumbs):
        ...
        execute_charters[i] = execute_charters[i] + "Дата создания`"
        values_charters = values_charters + 'NOW());'
        execute_charters[i] = execute_charters[i] + ") VALUES (" + values_charters
    return execute_charters

def gen_execute_params(List, tablename,filename):
    execute_charters = "INSERT INTO `" + filename + "`.`" + tablename + "`(`"
    values_charters = ""
    for params in List:
            execute_charters = execute_charters + str(params[0]) +  "`, `"
            values_charters = values_charters + "'" + str(params[1]) + "', "
    execute_charters = execute_charters + "Дата создания`"
    values_charters = values_charters + 'NOW());'
    execute_charters = execute_charters + ") VALUES (" + values_charters
    return execute_charters

def load_executeparametras(charters, disk, filename):
    execute_charters = gen_execute_params(charters, "all configuration", filename)
    execute_disk = gen_execute_disk(disk, "disk configuration", filename)
